...
class PaymentActivity : BaseActivity(), View.OnClickListener, RadioGroup.OnCheckedChangeListener {
    ...
    override fun onClick(p0: View?) {
        when(p0!!.id)
        {
            R.id.btnConfirmOrder -> { if(checkValidation()) { paymentViewModel.placeOrderApi(this,prefs.jwtToken!!,payment_type!!,"123456",prefs.coupon_code!!,tvShippingCharges.text.toString(),address_id!!) }  }
            R.id.tvAddress-> {
    ...
    override fun onCheckedChanged(group: RadioGroup?, checkedId: Int) {
        ...
        {
            R.id.rbOnline ->{ payment_type=ParamEnum.ONLINE.theValue().toString() }
            R.id.rbCOD ->{ payment_type=ParamEnum.COD.theValue().toString() }
        }
    }
