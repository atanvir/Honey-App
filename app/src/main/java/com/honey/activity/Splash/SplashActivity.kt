...
import com.honey.utils.CommonUtils.Companion.DELAY_GIF_BACKGROUND
...
class SplashActivity: BaseActivity()
{
    ...
    private fun startGif() {
        try {
            Glide.with(this).asGif().load(R.drawable.splsh).listener(object : RequestListener<GifDrawable?> {
            ...
            override fun onResourceReady(resource: GifDrawable?, model: Any?, target: Target<GifDrawable?>?, dataSource: com.bumptech.glide.load.DataSource?, isFirstResource: Boolean): Boolean {
                Handler().postDelayed({ if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)  clMain.backgroundTintList=getColorStateList(R.color.app_theme_color) },DELAY_GIF_BACKGROUND)
                startIntent()
                return false
            }
            }).into(image)
